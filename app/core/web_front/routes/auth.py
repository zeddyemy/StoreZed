...
def sign_up():
    ...
    if current_user.is_authenticated:
        return redirect(redirect_url('web_front.index'))
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            try:
                ...
                role: Role = Role.query.filter_by(name=RoleNames.CUSTOMER).first()
                
                db.session.add_all([new_user, new_user_profile, new_user_address, new_user_wallet])
                db.session.commit()
                ...
                return redirect(redirect_url('web_front.login'))
    ...
    return render_template('web_front/pages/auth/register.html', form=form, page='auth')


## Route to Login
@web_front_bp.route("/login", methods=['GET', 'POST'])
def login():
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                if not next or urlparse(next).netloc != '':
                    next = url_for('web_front.index')
                
                if not user:
                    flash("Email/Username is incorrect or doesn't exist", 'error')
                    return render_template('web_front/pages/auth/login.html', form=form, page='auth')
...
## Route to Logout
@web_front_bp.route("/logout", methods=['GET', 'POST'])
@login_required
def logout():
    logout_user()
    flash("You have been Logged Out", 'success')
    return redirect(url_for('web_front.login'))
